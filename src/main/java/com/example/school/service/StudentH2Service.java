...
public class StudentH2Service implements StudentRepository {
    ...
    public Student addStudent(Student student) {
        ...
        return db.queryForObject("select * from student where studentName = ? and gender = ?", new StudentRowMapper(), student.getStudentName(), student.getGender());
    }

    @Override 
    public ArrayList<Student> addStudentsBulk(Student student) {
        db.update("insert into student(studentName, gender, standard) values (?,?,?)", student.getStudentName(), student.getGender(), student.getStandard());
        List<Student> studentBulkList = db.query("select * from student", new StudentRowMapper());
        ArrayList<Student> studentsBulk = new ArrayList<>(studentBulkList);
        return studentsBulk;
    }
